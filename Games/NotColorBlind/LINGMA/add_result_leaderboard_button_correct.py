...
# -*- coding: utf-8 -*-
"""
在ResultScene中添加排行榜按钮（使用正确的by_id参数）
"""
import asyncio
import aiohttp
import json
...
async def add_result_leaderboard_button():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if text_id:
            # 步骤7: 添加Text的RectTransform（使用by_id）
                ...
                if response.status == 200:
                    ...
                    for line in response_text.split('\n'):
                        ...
                        if line.startswith('data:'):
                            ...
                            structured = data.get('result', {}).get('structuredContent', {})
                            if structured and structured.get('success'):
                                print(f"✅ Text的RectTransform添加成功")
            
            # 设置Text的RectTransform属性（使用by_id）
                    ...
                    if response.status == 200:
                        ...
                        for line in response_text.split('\n'):
                            ...
                            if line.startswith('data:'):
                                ...
                                structured = data.get('result', {}).get('structuredContent', {})
                                if structured and structured.get('success'):
                                    print(f"  ✅ Text.RectTransform.{prop_name}设置成功")
