...
"""
修复场景中的按钮引用配置
1. LevelScene: 将ReturnButton引用到LevelController.returnButton
2. ResultScene: 将ResultLeaderboardButton引用到ResultController.leaderboardButton
3. ResultScene: 添加LeaderboardPanel Prefab实例并引用到leaderboardUI
"""
import asyncio
import aiohttp
import json
...
async def fix_level_scene(session, headers):
    ...
    # 加载LevelScene
    print("\n[1] 加载LevelScene...")
    result = await call_tool(session, headers, 'manage_scene', {'action': 'load', 'name': 'LevelScene'}, 1)
    ...
    if result.get('success') and result.get('data', {}).get('instanceIDs'):
        ...
        print(f"未找到ReturnButton: {result}")
        return False
    
    await asyncio.sleep(0.3)
    
    # 查找LevelController组件所在的对象
    print("\n[3] 查找LevelController...")
    result = await call_tool(session, headers, 'find_gameobjects', {
        'search_term': 'LevelController',
        'search_method': 'by_component'
    }, 3)
    
    if result.get('success') and result.get('data', {}).get('instanceIDs'):
        controller_id = result['data']['instanceIDs'][0]
        print(f"找到LevelController所在对象, ID: {controller_id}")
    else:
        # LevelController可能在Canvas上
        result = await call_tool(session, headers, 'find_gameobjects', {
            'search_term': 'Canvas',
            'search_method': 'by_name'
        }, 3)
        if result.get('success') and result.get('data', {}).get('instanceIDs'):
            controller_id = result['data']['instanceIDs'][0]
            print(f"LevelController应该在Canvas上, ID: {controller_id}")
        else:
            print(f"未找到LevelController")
            return False
    
    await asyncio.sleep(0.3)
    
    # 设置returnButton属性
    print("\n[4] 设置LevelController.returnButton引用...")
    result = await call_tool(session, headers, 'manage_components', {
        'action': 'set_property',
        'target': str(controller_id),
        'search_method': 'by_id',
...
async def fix_result_scene(session, headers):
    ...
    # 加载ResultScene
    print("\n[1] 加载ResultScene...")
    result = await call_tool(session, headers, 'manage_scene', {'action': 'load', 'name': 'ResultScene'}, 1)
    print(f"加载场景: {result.get('success', False)}")
    await asyncio.sleep(0.3)
    ...
    if result.get('success') and result.get('data', {}).get('instanceIDs'):
        ...
        print(f"未找到ResultLeaderboardButton: {result}")
        return False
    
    await asyncio.sleep(0.3)
    
    # 查找Canvas (ResultController应该在Canvas上)
    print("\n[3] 查找Canvas...")
    result = await call_tool(session, headers, 'find_gameobjects', {
        'search_term': 'Canvas',
        'search_method': 'by_name'
    ...
    # 查找或创建LeaderboardPanel
    print("\n[5] 查找LeaderboardPanel...")
    result = await call_tool(session, headers, 'find_gameobjects', {
    ...
    if result.get('success') and result.get('data', {}).get('instanceIDs'):
        ...
        print(f"找到LeaderboardPanel, ID: {panel_id}")
    else:
        print("未找到LeaderboardPanel，需要手动添加或实例化Prefab")
