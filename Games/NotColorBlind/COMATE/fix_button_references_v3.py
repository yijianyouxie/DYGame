...
async def fix_level_scene(session, headers):
    ...
    print("\n" + "="*60)
    print("修复 LevelScene (buildIndex=1)")
    print("="*60)
    
    # 使用buildIndex加载LevelScene
    print("\n[1] 加载LevelScene...")
    result = await call_tool(session, headers, 'manage_scene', {
        'action': 'load',
        'build_index': 1
    }, 1)
    print(f"加载结果: success={result.get('success', False)}")
    
    if not result.get('success'):
        print(f"错误: {result.get('error', result.get('code', 'unknown'))}")
        return False
    ...
    if button_id and canvas_id:
        ...
        print(f"设置结果: success={result.get('success', False)}")
...
async def fix_result_scene(session, headers):
    ...
    print("\n" + "="*60)
    print("修复 ResultScene (buildIndex=2)")
    print("="*60)
    
    # 使用buildIndex加载ResultScene
    print("\n[1] 加载ResultScene...")
    result = await call_tool(session, headers, 'manage_scene', {
        'action': 'load',
        'build_index': 2
